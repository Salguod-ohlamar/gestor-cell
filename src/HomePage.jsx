...
const HomePage = ({ onLoginClick }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            // --- Lógica de Cache ---
            const loadFromCache = () => {
                const cachedBanners = localStorage.getItem('boycell-cache-home-banners');
                const cachedProducts = localStorage.getItem('boycell-cache-home-products');
                const cachedServices = localStorage.getItem('boycell-cache-home-services');
            ...
            // --- Busca de dados online ---
            try {
                const [bannersRes, productsRes, servicesRes] = await Promise.all([
                ...
                if (bannersRes.ok) {
                    ...
                    setBanners(data);
                    localStorage.setItem('boycell-cache-home-banners', JSON.stringify(data));
                }

                if (productsRes.ok) {
                    const products = await productsRes.json();
                    localStorage.setItem('boycell-cache-home-products', JSON.stringify(products));
                    const featuredProducts = products.filter(p => p.destaque === true);
                    const groupedProducts = featuredProducts.reduce((acc, product) => {
                        const category = product.categoria || 'Outros';
                        if (!acc[category]) acc[category] = [];
                ...
                if (servicesRes.ok) {
                    ...
                    localStorage.setItem('boycell-cache-home-services', JSON.stringify(services));
                    setFeaturedServices(services.filter(s => s.destaque === true));
                }
            } catch (error) {
        ...
        <div className="bg-white dark:bg-gray-950 text-gray-800 dark:text-gray-100 font-sans leading-relaxed">
            ...
            <main className="container mx-auto px-4 py-8 md:py-16">
                ...
                <section id="produtos" className="mt-20">
                    ...
                    {loading && Object.keys(productsByCategory).length === 0 ? (<div className="text-center text-lg text-gray-500 dark:text-gray-400">Carregando produtos...</div>) : (
                ...
                <section id="servicos" className="mt-20">
                    ...
                    {loading && featuredServices.length === 0 ? (<div className="text-center text-lg text-gray-500 dark:text-gray-400">Carregando serviços...</div>) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
