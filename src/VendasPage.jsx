...
const VendasPage = ({ onLogout, currentUser }) => {
    ...
    const vendedorDashboardData = useMemo(() => {
        ...
        const vendasConsideradas = ['admin', 'root', 'vendedor'].includes(currentUser?.role)
            ? salesHistory
        ...
        const totalVendidoHoje = vendasHoje.reduce((acc, sale) => acc + Number(sale.total), 0);
        const totalVendidoMes = vendasMes.reduce((acc, sale) => acc + Number(sale.total), 0);
