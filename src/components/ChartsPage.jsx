                    ...
                    {chartsConfig.map((chart, index) => {
                        ...
                        switch(chart.id) {
                                ...
                                chartContent = (
                                    <div className="flex flex-col h-full">
                                        <div className="flex justify-center gap-2 mb-4 flex-shrink-0">
                                            <button onClick={() => setSalesChartPeriod('day')} className={`px-3 py-1 text-sm rounded-full transition-colors ${salesChartPeriod === 'day' ? 'bg-cyan-600 text-white' : 'bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'}`}>Dia</button>
                                            <button onClick={() => setSalesChartPeriod('week')} className={`px-3 py-1 text-sm rounded-full transition-colors ${salesChartPeriod === 'week' ? 'bg-cyan-600 text-white' : 'bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'}`}>Semana</button>
                                            <button onClick={() => setSalesChartPeriod('month')} className={`px-3 py-1 text-sm rounded-full transition-colors ${salesChartPeriod === 'month' ? 'bg-cyan-600 text-white' : 'bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600'}`}>Mês</button>
                                        </div>
                                        <div className="flex-grow">
                                            <ResponsiveContainer width="100%" height="100%"><BarChart data={salesByPeriodData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? "#4A5568" : "#E2E8F0"} /><XAxis dataKey="period" stroke={theme === 'dark' ? "#A0AEC0" : "#4A5568"} tickFormatter={(str) => { if (salesChartPeriod === 'month') return new Date(str + '-02').toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' }); return new Date(str).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }); }} minTickGap={20} /><YAxis stroke={theme === 'dark' ? "#A0AEC0" : "#4A5568"} tickFormatter={(value) => `R$${value >= 1000 ? `${value/1000}k` : value}`} /><Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#1A202C' : '#FFFFFF', border: `1px solid ${theme === 'dark' ? '#4A5568' : '#CBD5E0'}` }} labelStyle={{ color: theme === 'dark' ? '#E2E8F0' : '#1A202C' }} formatter={(value) => [value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }), "Total Vendido"]} labelFormatter={(label) => { if (salesChartPeriod === 'month') return new Date(label + '-02').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }); if (salesChartPeriod === 'week') return `Semana de ${new Date(label).toLocaleDateString('pt-BR', { dateStyle: 'short' })}`; return new Date(label).toLocaleDateString('pt-BR', { dateStyle: 'long' }); }} /><Legend wrapperStyle={{ paddingTop: '20px' }} /><Bar dataKey="total" fill="#FF8042" name="Total Vendido" /></BarChart></ResponsiveContainer>
                                        </div>
                                    </div>
                                );
                                break;
                            ...
                            case 'lowStock':
                                ...
                                break;
                            case 'category':
                                // Ordena os dados da maior para a menor quantidade
                                const sortedCategoryData = [...dashboardData.categoriaDistribution].sort((a, b) => b.value - a.value);
                                chartContent = (
                                    <BarChart data={sortedCategoryData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                        <CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? "#4A5568" : "#E2E8F0"} />
                                        <XAxis dataKey="name" stroke={theme === 'dark' ? "#A0AEC0" : "#4A5568"} tick={{ fontSize: 12 }} interval={0} angle={-30} textAnchor="end" height={70} />
                                        <YAxis stroke={theme === 'dark' ? "#A0AEC0" : "#4A5568"} allowDecimals={false} />
                                        <Tooltip contentStyle={{ backgroundColor: theme === 'dark' ? '#1A202C' : '#FFFFFF', border: `1px solid ${theme === 'dark' ? '#4A5568' : '#CBD5E0'}` }} labelStyle={{ color: theme === 'dark' ? '#E2E8F0' : '#1A202C' }} formatter={(value) => [`${value} unidades`, "Quantidade em Estoque"]} />
